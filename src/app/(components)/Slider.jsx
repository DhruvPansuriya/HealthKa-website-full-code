        ...
        <div className="relative h-[400px] lg:h-[600px] w-full overflow-hidden ">
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                {slides.map((_, index) => (
                    <div
                        key={index}
                        className={`w-2 md:w-3 h-2 md:h-3 rounded-full transition-all duration-500 ${index === currentIndex ? 'bg-blue_new' : 'bg-red_new'
                            }`}
                    ></div>
                ))}
            </div>
            ...
            <div className=' absolute left-8  md:left-16 lg:inset-0  w-[85%] lg:w-[70vw] h-[100%] lg:h-[450px] lg:ml-[8vw] flex flex-col justify-center lg:justify-end '>
                <div data-aos="fade-up" className='w-[90%] h-auto lg:h-[35%] text-4xl lg:text-5xl text-[#2C2D3F] font-bold'>We build products that open <a className='text-blue_new'>opportunities</a> & solve problems globally.</div>
            <div data-aos="fade-up" data-aos-delay="300" className='py-3 lg:pt-0 my-6 lg:mt-1 w-[95%] lg:w-[50%] h-auto text-sm lg:text-1xl text-[#2C2D3F] font-medium '>Share your unique insights, creativity, and a fearless approach to impossible challenges. Together, we can build solutions for all.</div>
                <div data-aos="fade-up" data-aos-delay="300" className=' w-[80%] h-auto lg:h-[20%] flex items-center justify-start space-x-7 '>
